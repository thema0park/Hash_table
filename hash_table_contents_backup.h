...
class my_map{
	...
	public:
		...
		V put(K key, V value)
		{
			...
			////////////////////////
			if((save_cnt/table_size) > 0.7){
				cout <<  "Maybe table is full.." << endl;
				cout << "rehashing execute.." << endl;
				rehashing();
			}
			////////////////////////
			...
			if(hash_table[hash1_key].first == key)
			{
				...
				hash_table[hash1_key].second = value;
				return "\0";
			}
		...
		V remove(K key){
			...
			if(flag==true){
				...
				if(hash_table[hash1_key].first==key)
				{
					...
					hash_table[hash1_key]=make_pair("\0","\0");
				...
				else
				{
					while(1)
					{
						...
						if(hash_table[hash2_key].first==key)
						{
							...
							hash_table[hash2_key]=make_pair("\0","\0");
		...
		vector<pair<K, V>> entrySet()
		{
			for(int i = 0; i < entrys.size(); i++)
			{
				cout << entrys[i].first << endl;
				cout << entrys[i].second << endl;
			}
			return entrys;
		}
		...
		double get_lf(){
			return (save_cnt/table_size);
		}

		double get_collision_cnt(){
